...
/**
 * Created by zhouyajun on 2015/9/17.
 */
public class NodeWatcher{
    private static final String PATH = "/consumers";
    public static void main(String[] args) throws Exception {
        CuratorFramework client = null;
        TreeCache cache = null;
        try {
            client = CuratorFrameworkFactory.newClient("119.254.97.32:2181", new ExponentialBackoffRetry(1000, 3));
            client.start();
            /*client.create().forPath("/yajunz");
    ...
    private static void list(TreeCache cache) {
        if (cache.getCurrentChildren(PATH).size() == 0) {
            ...
            Map<String, ChildData> childDataMap = cache.getCurrentChildren(PATH);
            for(Iterator<String> iterator = childDataMap.keySet().iterator(); iterator.hasNext();){
                ChildData childData = childDataMap.get(iterator.next());
